...
export default function ReportPage() {
  ...
  function handleComplete(url: string, manifest: string | null) {
    setPdfUrl(url)
    setManifestId(manifest)
    setCopied(false)
  }
  ...
  async function handleCopyVerifyLink() {
    ...
    try {
      await navigator.clipboard.writeText(
        `${window.location.origin}/${locale}/verify/${manifestId}`,
      )
      setCopied(true)
    } catch {
      setCopied(false)
  ...
  const verifyUrl = manifestId ? `/${locale}/verify/${manifestId}` : null
      ...
      {pdfUrl ? (
        <div className="mt-8">
          <div className="rounded-md border border-border bg-surface-raised px-6 py-10 text-center shadow-elevation-1">
            ...
            {verifyUrl && (
              <div className="mx-auto mt-6 max-w-md rounded-sm border border-accent/30 bg-accent-muted/40 p-4 text-left">
                <p className="text-sm font-semibold text-accent">{t('verifyTitle')}</p>
